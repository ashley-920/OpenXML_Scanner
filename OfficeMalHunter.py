...
def save_decompressed_macro( name, buffer, idx ):
    ...
    try:
        if name == '': name = 'VBA_file'
        f = open( "%s\%s" % (g_macro_dir, name+".txt"), 'wb' )
...
def print_stream_info( f, stg ,des_dir):
    """print stream info
    ...
    if stg is None:
        ...
        print_stream_info( None, ost,des_dir )
    else:
...
#                for c in s_info[0][0].encode( 'ascii', 'ignore' ):
                ...
                if s_info[0][1] - 1 > 3:
                    ...
                    if s_info[0][1] == 1:
                        ...
                        g_IndentNum += 1
                        print_stream_info( None, x_stg ,des_dir)
                        g_IndentNum -= 1
...
def main():
    ...
    if mode_flg == 1:
        ...
        print_stream_info( sys.argv[1], None,des_dir )
