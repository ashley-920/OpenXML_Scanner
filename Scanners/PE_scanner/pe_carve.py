...
def scan(fname):
    ...
    report+="scanning PE on file "+fname+"........\n"
    time = datetime.now().strftime("[%d %b %y @ %H:%M:%S]")
    report+="Scan started on "+fname+ " at "+time+"\n"
    ...
    for hit in list:
        ...
        if (10000 < pesize < 10000000) and PE_header[0:2] == "MZ":
            report+="Found at: 0x"+hex(hit)+" ("+str(pesize)+" bytes)\n"
            ...
            report+="Ignored PE header at 0x"+hit+"\n"
            log ("Ignored PE header at 0x%X" % hit)
        
    time = datetime.now().strftime("[%d %b %y @ %H:%M:%S]")
    report+="Scan ended on "+fname+" at "+time+"\n"
