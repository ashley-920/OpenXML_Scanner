...
def omh_shellcode_scan(g_f_cnt):
    ...
    print "shellcode_scanner2 Scanning Now!"
    report=report+ "\n[*] Scanning now...\n\n"
    print type(report)
    for i in xrange(g_f_size):
        ...
        if ( libc.memcmp( byref(g_FS30Sig1), g_f_cnt[i:], 5 ) == 0 or
            ...
            report=report+"FS:[30h] (Method 1) signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
        ...
        if ( unpack( 'B', g_f_cnt[i]   )[0] == 0x6A and
            ...
            report=report+"FS:[30] (Method 2) signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE
        ...
        if ( unpack( 'B', g_f_cnt[i]   )[0] == 0x33 and
            ...
            report=report+"FS:[30] (Method 3) signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE
        ...
        if ( unpack( 'B', g_f_cnt[i]   )[0] == 0x74 and
            ...
            report=report+"API-Hashing signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE
        ...
        if ( unpack( 'B', g_f_cnt[i]   )[0] == 0x55 and
            ...
            report=report+"Function prolog signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE
        ...
        if ( unpack( 'B', g_f_cnt[i]   )[0] == 0xFF and
            ...
            report=report+"PUSH DWORD[]/CALL[] signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE
        ...
        if ( unpack( 'B', g_f_cnt[i]   )[0] == 0xAC and
            ...
            report=report+"LODSB/STOSB XOR decryption signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE
        ...
        if ( unpack( 'B', g_f_cnt[i]   )[0] == 0xAC and
            ...
            unpack( 'B', g_f_cnt[i+3] )[0] == 0xAA ):
            report=report+"LODSB/STOSB ADD decryption signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE
        ...
        if ( unpack( 'B', g_f_cnt[i]   )[0] == 0xAC and
            ...
            unpack( 'B', g_f_cnt[i+3] )[0] == 0xAA ):
            report=report+"LODSB/STOSB SUB decryption signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE
        ...
        if ( unpack( 'B', g_f_cnt[i]   )[0] == 0xAC and
            ...
            unpack( 'B', g_f_cnt[i+3] )[0] == 0xAA ):
            report=report+"LODSB/STOSB ROL decryption signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE
        ...
        if ( unpack( 'B', g_f_cnt[i]   )[0] == 0xAC and
            ...
            unpack( 'B', g_f_cnt[i+3] )[0] == 0xAA ):
            report=report+"LODSB/STOSB ROR decryption signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE
        ...
        if ( unpack( 'B', g_f_cnt[i]   )[0] == 0x66 and
            ...
            report=report+"LODSW/STOSW XOR decryption signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE
        ...
        if ( unpack( 'B', g_f_cnt[i]   )[0] == 0x66 and
            ...
            unpack( 'B', g_f_cnt[i+6] )[0] == 0x66 and
            unpack( 'B', g_f_cnt[i+7] )[0] == 0xAB ):
            report=report+"LODSW/STOSW ADD decryption signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE
        ...
        if ( unpack( 'B', g_f_cnt[i]   )[0] == 0x66 and
            ...
            unpack( 'B', g_f_cnt[i+6] )[0] == 0x66 and
            unpack( 'B', g_f_cnt[i+7] )[0] == 0xAB ):
            report=report+"LODSW/STOSW SUB decryption signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE
        ...
        if ( unpack( 'B', g_f_cnt[i]   )[0] == 0xAD and
            ...
            report=report+"LODSD/STOSD XOR decryption signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE
        ...
        if ( unpack( 'B', g_f_cnt[i]   )[0] == 0xAD and
            ...
            unpack( 'B', g_f_cnt[i+6] )[0] == 0xAB ):
            report=report+"LODSD/STOSD ADD decryption signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE
        ...
        if ( unpack( 'B', g_f_cnt[i]   )[0] == 0xAD and
            ...
            unpack( 'B', g_f_cnt[i+6] )[0] == 0xAB ):
            report=report+"LODSD/STOSD SUB decryption signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE

        if libc.memcmp( byref(g_FldzSig), g_f_cnt[i:], 6 ) == 0:
            report=report+"FLDZ/FSTENV [esp-12] signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE
        ...
        if ( libc.memcmp( byref(g_CallPopSig1), g_f_cnt[i:], 6 ) == 0 or
            ...
            report=report+"CALL next/POP signature found at offset: 0x"+str(i)+"\n"
            # report=report.join(output)
            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
            g_power += RATING_CODE
                    ...
                    if ( jmp_off + call_va + 5 < g_f_size):
                        if( unpack( 'B', g_f_cnt[jmp_off+call_va+5] )[0] == 0x58 or
                           ...
                           unpack( 'B', g_f_cnt[jmp_off+call_va+5] )[0] == 0x5F ):
                            report=report+"JMP [0xEB]/CALL/POP signature found at offset: 0x"+str(i)+"\n"
                            # report=report.join(output)
                            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
                            g_power += RATING_CODE
            ...
            #                call_va  = unpack( '<L', g_f_cnt[jmp_off+1:jmp_off+5] )[0]  # python is much simple
                    ...
                    if ( jmp_off + call_va + 5 < g_f_size):
                        if( unpack( 'B', g_f_cnt[jmp_off+call_va+5] )[0] == 0x58 or
                           ...
                           unpack( 'B', g_f_cnt[jmp_off+call_va+5] )[0] == 0x5E or
                           unpack( 'B', g_f_cnt[jmp_off+call_va+5] )[0] == 0x5F ):
                            report=report+"JMP [0xE9]/CALL/POP signature found at offset: 0x"+str(i)+"\n"
                            # report=report.join(output)
                            if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
                            g_power += RATING_CODE
        ...
        if ( libc.memcmp( c_char_p("MZ"), g_f_cnt[i:], 2 ) == 0 ):
            ...
            if ( libc.memcmp( c_char_p("PE"), g_f_cnt[i+pe_off:], 2 ) == 0):
                report=report+"unencrypted MZ/PE signature found at offset: 0x"+str(i)+"\n"
                # report=report.join(output)
    ...
    #         report=report+"NOP slides signature found at offset: 0x"+str(i)+"\n"
    #         # report=report.join(output)
    #         if debug_flg == 1: print_opcodz( g_f_cnt[i:] )
    ...
    #             report=report+"API-Name "+api+" string found at offset: 0x"+str(i)+"\n"
    #             # report=report.join(output)
    #             if debug_flg == 1: dump_data( "PE-File", g_f_cnt[i:], 0x100 )
    #             g_power += RATING_STRS
    ...
    #         report=report+"Embedded OLE signature found at offset: 0x"+str(i)+"\n"
    #         # report=report.join(output)
    #         if debug_flg == 1: dump_data( "PE-File", g_f_cnt[i:], 0x100 )
    #         g_power += RATING_OLENOP
